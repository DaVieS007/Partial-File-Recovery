...
int main(int argc, char *argv[])
{
    ...
    if(argc < 3)
    {
        ...
        fprintf(stderr,"Even the most secure filesystems like ZFS could have data corruption. This software intended to retrieve the data regardless of data loss with many additional options.\n");
        fprintf(stderr,"In our research, we concluded that restoring files or block devices even with data loss, does not mean that the data must have corrupted or unusable at all.\n\n");
        fprintf(stderr,"From version 2.0 we added differential method to allow minimize writes. This could very handy when write operation is slower than double read.\nIt can work for restoring old snapshots or differential clones back to itself.\n");
        fprintf(stderr,"Author: Viktor Hlavaji aka DaVieS, davies@npulse.net | Licensed under: GNU/GPLv3\033[m\n\n");

        fprintf(stderr,"\033[93mPlease make a backup and then test before you start any live operation, software comes without any warranty.\033[m\n\n");
        

        fprintf(stderr,"\033[1mUsage: %s [source_file] [dest_file] [differential = 0] [throttle = 0] [try = 1] [fill = 00] [skip = 512] [seek = 0] [endpos = 0]\033[m\n\n",argv[0]);
        ...
        fprintf(stderr," - differential: (Optional) 0 = no, 1 = yes\n");
    ...
    if (dst == NULL)
    {
        if(differential)
        {
            fprintf(stderr,"Unable to open destination file for differential writing: %s\n\n",argv[2]);
            fclose (src);
            return EXIT_FAIL;
        }
